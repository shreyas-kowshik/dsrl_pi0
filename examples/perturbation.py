...
# -----------------------------------------
# LIBERO-PRO environment setup for training
# -----------------------------------------

_EVAL_CONFIG_PATH = "LIBERO-PRO/evaluation_config.yaml"


def setup_libero_pro_env(task_suite_name: str, variant) -> str:
    """Set up a LIBERO-PRO perturbation environment before task suite initialization.

    Reads the evaluation config, overrides perturbation flags with values from
    ``variant``, creates perturbed BDDL files and init states if needed, and
    returns the (possibly updated) task_suite_name.

    Args:
        task_suite_name: Base task suite name (e.g. "libero_10").
        variant: Namespace/dict with boolean perturbation flags
            (use_swap, use_object, use_language, use_task, use_environment).

    Returns:
        The task suite name to use for benchmark lookup. Unchanged when no
        perturbation flags are active; otherwise a modified name such as
        "libero_10_swap" or "libero_10_temp".
    """
    use_swap = bool(getattr(variant, "use_swap", False))
    use_object = bool(getattr(variant, "use_object", False))
    use_language = bool(getattr(variant, "use_language", False))
    use_task = bool(getattr(variant, "use_task", False))
    use_environment = bool(getattr(variant, "use_environment", False))

    if not any([use_swap, use_object, use_language, use_task, use_environment]):
        logging.info("No LIBERO-PRO perturbation flags set. Using standard evaluation.")
        return task_suite_name

    with open(variant.eval_config_path, "r", encoding="utf-8") as f:
        evaluation_cfg = yaml.safe_load(f)

    # Override YAML flags with CLI values
    evaluation_cfg["use_swap"] = use_swap
    evaluation_cfg["use_object"] = use_object
    evaluation_cfg["use_language"] = use_language
    evaluation_cfg["use_task"] = use_task
    evaluation_cfg["use_environment"] = use_environment
    ...
    # Multiple perturbation flags -> combined "_temp" environment
    if sum([use_swap, use_object, use_language, use_task, use_environment]) > 1:
        ...
        init_file_path = evaluation_cfg.get("init_file_dir", "") + task_suite_name + "_temp/"
        ...
        if not os.path.exists(bddl_file_path) or not os.path.exists(init_file_path):
            ...
            create_env(configs=evaluation_cfg)
        else:
        ...
        new_task_suite_name = task_suite_name + "_temp"
        logging.info(f"LIBERO-PRO: using combined perturbation environment '{new_task_suite_name}'")
        print(f"LIBERO-PRO: using combined perturbation environment '{new_task_suite_name}'")
        return new_task_suite_name

    # Single perturbation flag
    if use_swap:
    ...
    # Paths for the correctly-named perturbation directories
    bddl_parent = str(Path(evaluation_cfg["bddl_files_path"]).parent)
    init_parent = evaluation_cfg.get("init_file_dir", "").rstrip("/")
    bddl_named = os.path.join(bddl_parent, task_suite_name + "_" + perturb_suffix)
    init_file_path = os.path.join(init_parent, task_suite_name + "_" + perturb_suffix)

    if not os.path.exists(bddl_named):
        # Bypass create_env() (which always writes to _temp AND runs the slow
        # generate_init_states.py). We only need BDDL files; .pruned_init files
        # are not used in our eval loop (env.reset() handles initialisation).
        flags = PerturbFlags(
        ...
        ood_task_configs = evaluation_cfg.get("ood_task_configs", {})

        # process_bddl_file_mixed always writes to {input}_temp
        temp_output_dir = process_bddl_file_mixed(
            input_dir=evaluation_cfg["bddl_files_path"],
            task_suite_name=task_suite_name,
            flags=flags,
            configs=ood_task_configs,
            seed=evaluation_cfg.get("seed", None),
        )
        shutil.copytree(temp_output_dir, bddl_named)
        print(f"[LIBERO-PRO] Created perturbed BDDL dir: {bddl_named}")

    # Create empty init dir so any path-existence checks in the benchmark pass.
    # Actual init states are generated on demand via env.reset().
    os.makedirs(init_file_path, exist_ok=True)

    new_task_suite_name = task_suite_name + "_" + perturb_suffix
    logging.info(f"LIBERO-PRO: using single perturbation environment '{new_task_suite_name}'")
    print(f"LIBERO-PRO: using single perturbation environment '{new_task_suite_name}'")
    return new_task_suite_name