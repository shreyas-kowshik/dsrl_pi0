#!/usr/bin/env python
"""Standalone evaluation script for Residual PARL checkpoints.

Loads a trained Residual PARL checkpoint, runs num_evals rollouts, generates
diagnostic plots for a subset of successful and failed trajectories, and writes
a summary CSV to output_dir.

Usage:
    python -m examples.evaluation.evaluate_residual_parl \\
...
def collect_eval_trajectory(variant, agent, env, agent_dp, rng):
    ...
    Args:
        ...
        agent: Residual PARL agent (already loaded from checkpoint).
...
def _create_agent(variant, sample_obs, sample_action, kwargs):
    ...
    algo = variant.get('algo', 'residual_parl')
...
def run_evaluation(variant):
    ...
    print('=' * 60)
    print('RESIDUAL PARL EVALUATION')
    print('=' * 60)
    ...
    print(f'  algo           : {variant.get("algo", "residual_parl")}')
    print('=' * 60)
    ...
    print(f'Agent created: {variant.get("algo", "residual_parl")}')
...
if __name__ == '__main__':
    parser = argparse.ArgumentParser(
        description='Evaluate a Residual PARL checkpoint.'
    )
    ...
    # Algorithm
    parser.add_argument('--algo', default='residual_parl', type=str,
